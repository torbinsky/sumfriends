...
public class AuthController extends Controller {
	...
	public Result login() {
		return ok(sumfriends.views.html.login.render(null));
	}
	...
	private Result getResultForAuthorizedAccount(String token){
		if(token != null){
			createSession(token);
			return redirect(sumfriends.controllers.routes.Application.index());
		}
		
		return unauthorized(sumfriends.views.html.login.render("Invalid email/password"));
	}
	...
	public Result signup() {
		return ok(sumfriends.views.html.signup.render(null));
	}
	...
	private @Nullable Result validateSignup(String email, String summonerName, String pass1, String pass2){
		if(email == null || email.trim().length() < 6){
			return badRequest(sumfriends.views.html.signup.render("Invalid email address"));
		}
		
		if(summonerName == null || summonerName.trim().length() < 4){
			return badRequest(sumfriends.views.html.signup.render("Invalid summoner name"));
		}
		
		if(pass1 == null || pass1.trim().length() < 6){
			return badRequest(sumfriends.views.html.signup.render("Your password must be at least 6 characters"));
		}
		
		if(!pass1.equals(pass2)){
			return badRequest(sumfriends.views.html.signup.render("Passwords do not match"));
		}
	...
	private Result getResultForAccountSignup(Either<UserAccount, String> result){
		if(result.left.isDefined()){
			...
			return badRequest(sumfriends.views.html.signup.render(result.right.get()));
		}
	}
